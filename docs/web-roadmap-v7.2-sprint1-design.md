...
| 总产物（JS）| 930.92 KB |
| 总产物（gzip）| **289.11 KB** |
| chunk 数 | **1 个**（所有代码打入单个 index-*.js） |
| build 警告 | "Some chunks are larger than 500 kB" |

#### F9 后（lazy + manualChunks）

**Vendor chunks（首屏与所有页面共用）**：

| chunk | raw | gzip |
|-------|-----|------|
| vendor-react | 272.19 KB | 86.71 KB |
| vendor-motion | 124.77 KB | 40.78 KB |
| vendor-radix | 111.02 KB | 35.79 KB |
| vendor-utils | 67.18 KB | 24.41 KB |
| vendor-icons | 23.82 KB | 8.67 KB |
| vendor-query | 24.85 KB | 8.16 KB |
| 入口 index-*.js | 51.19 KB | **15.68 KB** |
| **首屏 vendor 合计** | **674.22 KB** | **220.20 KB** |

**Page chunks（按需加载，命中后无网络）**：

| 页面 | raw | gzip |
|------|-----|------|
| home | 32.51 KB | 11.01 KB |
| report | 33.19 KB | 10.24 KB |
| milestone | 20.98 KB | 8.62 KB |
| family | 20.08 KB | 6.65 KB |
| vaccine | 18.06 KB | 5.19 KB |
| record（页面+组件） | 16.59 KB | 5.84 KB |
| jaundice | 15.20 KB | 5.73 KB |
| growth | 13.70 KB | 5.18 KB |
| ai-assistant | 11.35 KB | 4.18 KB |
| profile | 8.70 KB | 3.14 KB |
| baby | 7.85 KB | 3.00 KB |
| discover | 6.00 KB | 2.48 KB |
| settings | 5.85 KB | 2.37 KB |
| login | 4.49 KB | 2.15 KB |
| register | 3.61 KB | 1.55 KB |
| wechat-callback | 1.74 KB | 0.96 KB |
...
| 首屏首页加载 gzip（vendor + entry + home） | 289.11 KB | **231.21 KB** | **↓ 20.0%** |
| 路由切换增量（典型 page） | — | 2-11 KB gzip | 新增能力 |
| chunk 数 | 1 | **49** | 长缓存命中率显著提升 |
| build 警告 | 有（>500KB）| **无** | ✓ |
| dist/stats.html | — | ✓（ANALYZE=true 时） | 新增能力 |

**关键洞察**：
- 入口 chunk gzip 从 289KB 降到 15.68KB（核心改进），用户访问任意页面时业务代码仅需下载该页 chunk
- vendor 拆为 6 组后，长缓存策略下，更新业务代码不会让用户重新下载 React / Radix / framer-motion
- 报告页 / 成长页等"重图表"页面的代码不会拖累首屏首页
