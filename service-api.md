...
- `success=false` 时 `error.code` 为业务错误码（如 `USER_NOT_FOUND` / `FAMILY_NOT_FOUND` / `PERMISSION_DENIED` / `RATE_LIMITED` / `INVALID_CODE` / `CODE_EXPIRED` / `ALREADY_MEMBER` / `SOLE_ADMIN` / `INTERNAL_ERROR` / **v4.3.1 新增：** `ALREADY_IN_FAMILY` (createFamily 已在家庭中) / `INVALID_ROLE` (updateMemberRole 角色白名单)），`error.message` 为中文提示
...
| 方法 | 路径 | 返回 | 说明 |
|------|-----|------|------|
| `createBaby(familyId, name, gender, birthDate, avatar)` | `[cloud]` | `Promise<Baby>` | 云函数 `createBaby` action；**v4.3.1+ 仅 admin** 可创建；自动写入 `_openid = 调用者 openid` 使创建者后续可 updateBaby |
| `deleteBaby(babyId, familyId)` | `[cloud]` | `Promise<Object>` | 云函数 `deleteBaby` action；**v4.3.1+ 仅 admin**；**级联删除** 关联的 `records` / `vaccine_records` / `milestone_records`；支持 `cursor` 断点续传（大数据量） |
| `getBabiesByFamilyId(familyId)` | `[direct]` | `Promise<Array>` | 直连，安全规则 `get()` 校验通过；双路径（where babyId + families.babies 批量 in） |
| `getBabyById(babyId)` | `[cloud]` | `Promise<Baby\|null>` | **v4.3.1 Hotfix 改走云函数** `familyOperation/getBabyById` action。原直连方案受 `babies.read` 安全规则 `memberOpenids contains auth.openid` 约束，当 `memberOpenids` 未同步时必然失败。云函数内用 admin SDK 读取后在业务层校验 `baby.familyId === user.familyId` + `isMember`。不存在或无权限返回 `null` |
| `updateBaby(babyId, data)` | `[cloud]` | `Promise<Object>` | **v4.3.1 Hotfix3 改走云函数** `familyOperation/updateBaby` action。admin SDK 绕过 `doc._openid == auth.openid` 规则；业务层校验：同家庭 + 非 viewer 角色；字段白名单 `name/gender/birthDate/avatar`，自动写 `updatedAt/updatedAtTs` |
...
### ⚠️ `deleteBaby` 契约（v4.3.1 升级为分批删除）

`deleteBaby` v4.3.1 起会级联删除关联的 records / vaccine_records / milestone_records，以避免孤儿数据。大数据量时支持 cursor 断点续传：

```javascript
// 云函数返回：
{ success: true, data: { status: 'in_progress', cursor, progress: { records, vaccine, milestone } } }  // 未删完
{ success: true, data: { status: 'succeeded', deletedBabyId, records, vaccine, milestone } }            // 删完
```

**调用方（客户端 BabyService 不做自动循环，业务层需按 status 分支处理，或使用 `settings.clearAllCloudData` 的循环模式）**：
...
while (true) {
  const res = await wx.cloud.callFunction({
    name: 'familyOperation',
    data: { action: 'deleteBaby', params: { babyId, familyId, cursor } }
  });
  const data = res.result.data;
  if (data.status === 'succeeded') break;
  if (data.status === 'in_progress') { cursor = data.cursor; continue; }
