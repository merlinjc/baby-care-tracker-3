...
- [ ] **T-1.2** 实现测试数据定义 `lib/test-data.js`
...
- [ ] **T-2.1** 复制 familyOperation 内部函数到 `lib/family-operations.js`
...
- [ ] **T-2.2** 实现安全规则模拟器 `lib/rule-simulator.js`
...
- [ ] **T-3.1** 实现 `modules/m06-createFamily.js` — 5 条
  - CF-01~CF-05：正常创建、Ghost 用户、缺参数、邀请码格式、过期时间
  - 每条 reset 后调用 `createFamily()`，验证返回值和数据库状态
  - _需求：模块 6 全部 5 条_

- [ ] **T-3.2** 实现 `modules/m07-joinFamily.js` — 14 条
...
- [ ] **T-3.12** 实现 `modules/m17-errorHandling.js` — 5 条
...
- [ ] **T-4.1** 实现 `modules/m01-05-securityRules.js` — 40 条
...
- [ ] **T-4.2** 实现 `modules/m18-crossFamily.js` — 10 条
...
- [ ] **T-4.3** 实现 `modules/m19-stateChange.js` — 6 条
