...
- [x] **T-1.1** FR-6 删除 `AuthService.getOpenId()` 死代码
...
- [x] **T-1.3** FR-8 `family.js` 页面层改用 `familyService.getFamilyDetail`
...
- [x] **T-1.4** FR-9 `pages/auth/auth.js` 单例规范化
...
- [x] **T-2.1** FR-1 更新 `architecture.md`
...
- [x] **T-2.2** FR-2 更新 `service-api.md`
...
- [x] **T-2.4** FR-4 `coding-conventions.md` 新增 §8 数据库操作约束
  - 在现有 §7 权限体系之后插入 §8
...
- [x] **T-3.1** FR-5 回溯标记 v4.2 两个 spec 为已完成
  - `specs/v4.2-cloud-function-gateway/tasks.md` 头部状态 → `✅ 已完成（2026-04-17）`
  - `specs/v4.2-cloud-function-gateway/tasks.md` 所有 `- [x]` → `- [x]`
  ...
  - `specs/v4.2-e2e-security-tests/tasks.md` 所有 `- [x]` → `- [x]`
...
- [x] **T-3.2** 更新 `CHANGELOG.md` 新增 v4.2.2 区块
...
| v4.2 spec 的 tasks 清单过长，逐条勾选耗时 | 中 | 低 | 使用编辑器多光标批量替换 `- [x]` → `- [x]` |
