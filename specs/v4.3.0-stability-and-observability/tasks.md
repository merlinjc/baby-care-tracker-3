...
- [ ] **T-1.1** 新建 `miniprogram/utils/family-context.js`
...
- [ ] **T-1.2** 新建 `miniprogram/services/permission-guard.js`
...
- [ ] **T-2.1** `RecordService.createRecord` 离线路径补 `createdBy`
...
- [ ] **T-2.2** `SyncService.executeOperation` 时间戳规整
...
- [ ] **T-2.3** `RecordService` 全量 familyId 改造 + FamilyContext 接入
...
- [ ] **T-2.4** 其他模块 familyId 改走 FamilyContext
...
- [ ] **T-2.7** `RecordService` CRUD 接入 PermissionGuard
...
- [ ] **T-3.1** 建立 `cloudfunctions/familyOperation/` 新目录结构
...
- [ ] **T-3.3** 重写 `index.js` 为 dispatch 入口
...
- [ ] **T-5.1** `e2eSecurityTest` 重跑 + 补 v4.3 新用例
  - 部署最新云函数
...
- [ ] **T-5.2** 手动冒烟测试清单
  ...
  - [ ] 管理员清除 3000+ 条记录 → 自动多次调用直至完成
  - [ ] 暗色模式切换 / TabBar 30s 节流 / 分享链接 / 彩蛋等既有功能不受影响
...
- [ ] **T-5.4** 版本号全量同步（workflow §3.3.4 MINOR 级 8 项）
...
- [ ] **T-5.5** 更新 tasks.md 状态 + 本 spec 目录标记 ✅
